...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (!openAiKey && !geminiKey) {
      ...
      return runSimulationStream(res, userMessage);
    }
 ...
 */
async function runSimulationStream(res: express.Response, prompt: string) {
  res.setHeader('Content-Type', 'application/octet-stream');
  res.setHeader('Cache-Control', 'no-cache');
  res.setHeader('Connection', 'keep-alive');
...
How can I help you build your next generative web application today?`;
  ...
  // Stream text word-by-word with small random delays
  const words = responseText.split(' ');
  for (let i = 0; i < words.length; i++) {
    const chunk = words[i] + (i === words.length - 1 ? '' : ' ');
    res.write(chunk);
    // Simulate natural typing delay (approx 30-70ms per word)
